...
The df file has the excel table that is the dataframe for the regression models in the analysis.

Coding.py contains all coding that was used. Creating the dataframe needed many additional files, so those files are in the sub-folder Inputs.

The Output subfolder is for any output that Coding.py produces.

For the AI trends data, there is a separate folder AI Google Trends that contains the code for getting that dataset.

Important: The df contains the AI readiness index as a variable, but it was not included in the regression at the end.

-- Literature folder --

All literature that had pdfs and where it makes sense to include one (for example the sources for Google Trends is not in there) are inside this folder.
Organization of this folder could be improved still, since the names of the files are confusing sorry.
