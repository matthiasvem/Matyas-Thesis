...
model_full = smf.ols(
    formula="""
    gdp_pc_growth ~ ai_trends_std + hdi + ai_x_hdi + investment + fdi + trade_openness + inflation
    + C(country) + C(year)
    """,
    data=df_full
).fit(cov_type="HC1")  # heteroskedasticity-robust SEs
...
main_vars = ["ai_trends_std", "hdi", "ai_x_hdi", "investment", "fdi", "trade_openness", "inflation"]
rows = []
for v in main_vars:
    r1 = get_row(model_base, v)
    r2 = get_row(model_ext, v)
    r3 = get_row(model_full, v)
    rows.append({
        "variable": v,
        ...
        "(3) Full controls coef": r3["coef"],
        "(3) Full controls SE": r3["se"],
        "(3) Full controls pval": r3["pval"],
    })
...
info = pd.DataFrame(
    {
        ...
        "(2) + Investment coef": ["Yes", "Yes", int(model_ext.nobs), round(model_ext.rsquared, 3)],
        "(2) + Investment SE": ["", "", "", ""],
        "(2) + Investment pval": ["", "", "", ""],

        "(3) Full controls coef": ["Yes", "Yes", int(model_full.nobs), round(model_full.rsquared, 3)],
        "(3) Full controls SE": ["", "", "", ""],
        "(3) Full controls pval": ["", "", "", ""],
    },
    index=["Country FE", "Year FE", "Observations", "R-squared"])

final_table = pd.concat([table, info])
final_table.to_excel("Outputs/regression_table.xlsx")
...
# Robustness4: HCI -> Cluster

df_cluster = df_full[["gdp_pc_growth","ai_trends_std","hdi","ai_x_hdi","investment","fdi","trade_openness","inflation","country","year"]].dropna()
robust_cluster = smf.ols(
    formula="""
    gdp_pc_growth ~ ai_trends_std + hdi + ai_x_hdi
    + investment + fdi + trade_openness + inflation
    + C(country) + C(year)
    """,
    data=df_cluster
).fit(cov_type="cluster",cov_kwds={"groups": df_cluster["country"]})
...
with pd.ExcelWriter("Outputs/robustness_checks.xlsx") as writer:
    ...
    pd.DataFrame({"Coefficient": robust_cluster.params,"Std.Error": robust_cluster.bse,"P-value": robust_cluster.pvalues}).to_excel(writer, sheet_name="Clustered_SE")
